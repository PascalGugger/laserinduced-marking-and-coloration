%%NC-Code-Berechnung für Application
function NC_Code_Berechnung_app(P,Scangeschwindigkeit,bitmap_dithered,Groesse,Linienabstand,SkywriteLaenge,FileName,f,ZielOrdner)
tic
Groesse = Groesse*f;
[x,y] = size(bitmap_dithered);
Titel = [FileName,'_',num2str(P),'V_',num2str(Scangeschwindigkeit),'mms_',num2str(Linienabstand),'mm_bw_dithered_',num2str(Groesse*y),'mm_x_',num2str(Groesse*x),'mm_red_',num2str(f),'_NC.txt'];
Bahnanzahl = Groesse/Linienabstand; %Bahnen pro Pixelbreite
%fullfile('C:\Users\Pascal Gugger\OneDrive\Semesterarbeit\Matlab_Code\bitmap_scanner_bw\NC_Codes\',Titel);
fid = fopen(fullfile(ZielOrdner,Titel),'W'); 
fprintf(fid,['G90','\r\n']);
fprintf(fid,['G359','\r\n']);
fprintf(fid,['VELOCITY ON','\r\n']);

bar = waitbar(0,'NC-Code wird berechnet...');   %Ladebalken erstelle

leer = ceil((2*SkywriteLaenge)/Groesse);
bitmap_dithered = [ones(x,1),bitmap_dithered,ones(x,1)];
...
%type (['C:\Users\Pascal Gugger\OneDrive\Semesterarbeit\Matlab_Code\bitmap_scanner_bw\NC_Codes\',Titel]);
toc
end